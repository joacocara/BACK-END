...
function getCartById(id){
    // Método que retorna el carrito seleccionado usando su ID
    let isPresent = false
    let location = -1
    ...
    for (let i = 0; i < carts.length; i++) {
        if (carts[i].id == id) {
            isPresent = true
            location = i
            break  // Sal del bucle una vez encontrado el carrito
        }
    }
...
function addProdToCart(cid, pid) {
    ...
    if (isPresent) {
        ...
        fs.writeFileSync(ruta, JSON.stringify(cart), function (err) {
            if (err) throw err
        });
        return "0"
    }
...
router.post('/:cid/product/:pid',(req,res)=>{
    ...
    if (result=="01"){return res.status(400).json({error:'Cart no funciona'})}
    if (result=="10"){return res.status(400).json({error:'Prod no funciona'})}
